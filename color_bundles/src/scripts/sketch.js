...
class ColorBundle {
  constructor(x, y, colorStart, maxVar, len = Math.random() * 10 + 15, numLines = Math.random() * 10 + 5) {
    ...
    for (let i = 0; i < this.numLines; i++) {
      ...
      // const r = ((Math.random() * (maxVar * 2) - maxVar) + colorStart) % 255;
      // const g = ((Math.random() * (maxVar * 2) - maxVar) + colorStart) % 255;
      // const b = ((Math.random() * (maxVar * 2) - maxVar) + colorStart) % 255;
      // this.lines.push({
      //   rot,
      //   color: [r, g, b]
      // })
...
export function sketch(p, elem) {
  ...
  function makeBundle(start = p.random(0, 255), maxVar = 50) {
    return new ColorBundle(0, 0, start, maxVar);
  }

...
export default function floorSketch(p, elem) {
  ...
  p.setup = () => {
    ...
    noiseyCircles = [whAvg / 2, whAvg / 4, whAvg / 8].map((rad) => {
      ...
      const ang = p.random(0, 359);
      const color = p.color(p.random(0, 360), 25, 50, .5);
      return new NoiseCircle(cX + offset.x, cY + offset.y, rad, 30, ang, color);
    });
  };
  ...
  p.draw = () => {
    ...
    p.stroke(white);
    p.strokeWeight(6);
    p.noiseDetail(2, p.map(p.minute(), 0, 59, 0, 1));
